...
// Detect if image looks like a Collectr screenshot (tall portrait, 2 col grid)
function isCollectrScreenshot(img) {
  return img.naturalWidth >= 600 && img.naturalHeight >= 1200 && img.naturalHeight > img.naturalWidth * 1.5;
}
...
function initUploadTab() {
  ...
  async function processFiles(files) {
    ...
    if (!pendingCards.length) {
      setProgress("No cards detected. Make sure you uploaded Collectr screenshots.", 100);
      retryBtn.classList.remove("hidden");
      return;
    }
  ...
  async function enrichWithImages(cards, resizedImages) {
    // For each screenshot, crop all 6 grid positions
    // Then match to detected cards by order (API returns them top-left to bottom-right)
    const allCrops = [];
    ...
    for (const resized of resizedImages) {
      ...
      if (!isCollectrScreenshot(img)) {
        // Single card — use full image
        allCrops.push(resized.url);
        continue;
      }

      // Crop all 6 positions
